...
### Added
- **Architectural Guardrails**: Created `rules/SYSTEM_RULES.md` to enforce the immutability of `index.html` and the preservation of the `importmap` architecture.

## [1.1.0] - 2026-05-25

### Added
- **AI Health Scoring**: Individual project health scores (0-100) generated by the Analyst Agent, with color-coded status badges.
- **Strategic Timeline View**: A new "Gantt-like" visualization for the Tasks page, illustrating the critical path and sequential flow of operations.
- **Project Execution Roadmap**: Integrated a mini-timeline visualization within the Project Detail Modal to show internal task sequences and blocking dependencies.
- **Dependency Management**: Support for linking tasks together to establish a logical order of operations, helping founders identify "Next Best Actions" with more accuracy.

### Fixed
- **Architectural Cleanup**: Removed the forbidden `<script type="importmap">` from `index.html` to ensure strict compatibility with the Vite/React development guardrails.
- **Modal UX**: Improved backdrop blurring and entry animations for the Project Detail Modal.
